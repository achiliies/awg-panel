...
Все, что указано после `bash -s --`, передается установщику, поэтому флаги из разделов ниже работают и в этой форме:
...
Передавать URL напрямую в `sudo bash` — решение, к которому вполне обоснованно относиться с осторожностью. Ту же установку можно выполнить в два шага, если вы предпочитаете сначала получить файл:
...
Команда `sudo bash awg-panel.sh --help` выводит полный список всех флагов; то же самое делает `curl -fsSL .../get.sh | sudo bash -s -- --help`.
